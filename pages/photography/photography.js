...
Page({
  ...
  go_confirmorder(e) {
    ...
    if (!this.data.setmeal) {
      ...
      wx.navigateTo({
        url: '/pages/order/order?price=' + this.data.setmeal[this.data.index].wholesale_price + '&service_id=' + this.data.service_id + '&name=' + this.data.pet_list[this.data.index1].name + '&weight=' + this.data.pet_list[this.data.index1].weight + '&time=' + this.data.hour + '&member_pets_id=' + this.data.pet_list[this.data.index1].id  + '&shop_id=' + this.data.setmeal[this.data.index].shop_id + '&date_time=' + this.data.selectDate + '&mername=' + this.data.mername + '&exes=' + ['bath/expenses', 'bath/add', 'bath/store'] + '&specs=' + this.data.pet_list[this.data.index].specs + '&service_product_id=' + this.data.setmeal[this.data.index].id
      })
    }
  },
  //选择服务的宠物
  ...
  showerData(pets_id, name) {
    ...
    wx.request({
      url: url.api + `/ucs/v1/service/photo/match`, // 仅为示例，并非真实的接口地址
      data: {
      ...
      success: (res) => {
        ...
        if (res.data.code == 200) {
          setmeal[this.data.index].is_select = true
          this.setData({
            setmeal
          })
